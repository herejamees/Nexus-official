...
import { SwipeUpPanel, SwipeUpHint } from "./swipe-up-panel";
...
const PLAYFUL_STYLES = ["bottts", "fun-emoji", "adventurer", "lorelei", "thumbs", "pixel-art"];

function getPlayfulAvatar(seed: string): string {
  let hash = 0;
  for (let i = 0; i < seed.length; i++) {
    hash = (hash * 31 + seed.charCodeAt(i)) >>> 0;
  }
  const style = PLAYFUL_STYLES[hash % PLAYFUL_STYLES.length];
...
export function ChatInterface() {
      ...
      await api.sendMessage(
        ...
        () => {
          ...
          if (isFirstMessage && conversationId) {
            ...
            api.generateSmartTitle(conversationId, messageContent)
              .then(({ title }) => {
                updateConversationTitle(conversationId!, title);
              })
              .catch((error) => {
                console.error("Failed to generate smart title, using preview:", error);
                updateConversationTitle(conversationId!, titlePreview);
              })
              .finally(() => {
                loadConversations();
              });
  ...
  return (
    <div
      className="flex h-screen w-full flex-col bg-slate-50 dark:bg-slate-950 relative overflow-hidden"
      onTouchStart={(e) => { touchStartYRef.current = e.touches[0].clientY; }}
      onTouchEnd={(e) => {
        if (touchStartYRef.current === null) return;
        const deltaY = touchStartYRef.current - e.changedTouches[0].clientY;
        if (deltaY > 40 && !sidebarOpen) setSidebarOpen(true);
    ...
    >
      ...
      <SwipeUpPanel isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} />
      ...
      {/* Header */}
      <header className="sticky top-0 z-[100] flex items-center justify-between border-none bg-gradient-to-b from-white via-white/90 to-transparent dark:from-slate-950 dark:via-slate-950/90 dark:to-transparent px-4 md:px-6 py-2 transition-all duration-300">
        <div className="flex items-center gap-1.5 pointer-events-auto">
          <Button
            variant="ghost"
            size="icon"
            onClick={toggleTheme}
            className="h-8 w-8 text-slate-500 hover:text-slate-900 dark:hover:text-white transition-all duration-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full relative overflow-hidden group shrink-0"
          >
            <motion.div
              initial={false}
              animate={{ 
        ...
        <div className="flex items-center gap-1.5 pointer-events-auto">
           ...
           <Button
             ...
             className="h-8 w-8 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all button-animate"
           ...
           <Popover open={languageMenuOpen} onOpenChange={setLanguageMenuOpen}>
             <PopoverTrigger asChild>
               <button
                 ...
                 className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all button-animate bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700"
           ...
           {authLoading ? (
             ...
             <a
               href="https://account.actroo.com"
               target="_blank"
               rel="noopener noreferrer"
               className="h-9 w-9 rounded-full overflow-hidden ring-2 ring-transparent hover:ring-purple-400 transition-all cursor-pointer block shrink-0"
               title={user.username || user.firstName || "My Account"}
               data-testid="button-user-menu"
             >
               <img
                 src={getPlayfulAvatar(user.username || user.firstName || user.email || "user")}
                 alt={user.username || user.firstName || "User"}
                 className="h-full w-full object-cover bg-slate-100 dark:bg-slate-700"
             ...
             <button
               ...
               className={cn(
                 "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold shadow-sm hover:shadow-md transition-all button-animate",
      ...
      <main className="flex-1 overflow-hidden flex flex-col relative z-10">
        {/* Empty welcome state — no scroll, fills space */}
        {!isLoading && messages.length === 0 && (
          <div className="flex-1 flex flex-col items-center justify-center px-4 text-center">
            <motion.div
              className="h-16 w-16 bg-gradient-to-tr from-slate-200 to-slate-100 dark:from-slate-800 dark:to-slate-700 rounded-2xl mb-4 flex items-center justify-center shadow-inner shrink-0"
              animate={{ y: [0, -8, 0] }}
            ...
            <motion.p
              className="text-slate-500 dark:text-slate-400 max-w-sm text-sm leading-relaxed mb-5"
              initial={{ opacity: 0, y: 8 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              {starterPrompts.map((prompt, index) => (
                <button
                  ...
                  className="flex flex-col items-start gap-1.5 p-3 rounded-xl glass-card active:scale-95 transition-transform duration-150 text-left"
        ...
        {/* Messages list — only rendered when there are messages or loading */}
        {(isLoading || messages.length > 0) && (
        ...
        <div className="w-full px-4 z-20" style={{ paddingBottom: 'max(2.5rem, env(safe-area-inset-bottom, 2.5rem))' }}>
